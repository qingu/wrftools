...
def ncdump_wrftools(config):
    ...
    """ Designed tyo be called from run_forecast.py"""
    import wrftools
...
def ncdump(files,vars,global_atts,var_atts,coord_vars,output,log_name):
    ...
    for entry in output:
        ...
        if format=='txt' :
            ...
            write_csv_files(frame, entry['dir'], entry['fname'], entry['vars'],entry['dimspec'], entry['drop'], entry['values'],entry['rows'],entry['cols'],entry['sort-by'],entry['rename'],entry['float-format'], entry['na-rep'], log_name)
